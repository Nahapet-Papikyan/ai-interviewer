...
import { fadeUp, inView, stagger } from "@/components/landing/motion";
      ...
      >
        {reduce ? null : (
          <motion.div
            ...
            animate={{ x: [0, 18, -10, 0], y: [0, 14, -8, 0], opacity: [0.45, 0.7, 0.5, 0.45] }}
            transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
          />
        )}
        <div className="relative grid items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]">
          <motion.div initial={reduce ? false : "hidden"} whileInView="show" viewport={inView} variants={stagger(0.08, 0.05)}>
            <motion.div variants={fadeUp}>
              <Eyebrow>Free process assessment</Eyebrow>
            </motion.div>
            <motion.h2 variants={fadeUp} className="mt-4 max-w-xl text-3xl font-semibold tracking-tight text-cloud sm:text-5xl">
              Find out what your business can automate.
            </motion.h2>
            <motion.p variants={fadeUp} className="mt-4 max-w-xl text-base leading-7 text-mist">
            ...
            <motion.ul variants={fadeUp} className="mt-6 grid grid-cols-2 gap-3 text-sm text-cloud/90 sm:max-w-md">
